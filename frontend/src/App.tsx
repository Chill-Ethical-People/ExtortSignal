...
function AlertsPage({ alerts, onUpdated }: { alerts: Alert[]; onUpdated: () => Promise<void> }) {
  ...
  return (
    <div>
      ...
      <div className="mt-7 grid gap-3 sm:grid-cols-3 lg:grid-cols-6">{(["new", "investigating", "client_notified", "monitoring", "resolved", "dismissed"] as AlertStatus[]).map((status) => <div key={status} className="rounded-2xl border border-zinc-200 bg-white p-4"><AlertStatusBadge status={status} /><p className="mt-3 font-mono text-2xl font-semibold">{statusCounts[status] || 0}</p></div>)}</div>
      <div className="mt-7 flex gap-2" role="group" aria-label="Alert filters">
...
function AlertRow({ alert, divided }: { alert: Alert; divided?: boolean }) { return <div className={`group grid gap-4 p-5 transition hover:bg-zinc-50 sm:grid-cols-[auto_1.2fr_1fr_auto] sm:items-center ${divided ? "border-t border-zinc-200" : ""}`}><SeverityMark severity={alert.severity} /><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-zinc-900">{alert.claim_title}</p><AlertStatusBadge status={alert.status} /></div><p className="mt-1 text-xs text-zinc-500">Claim by {alert.threat_actor}</p></div><div><p className="text-sm font-medium text-zinc-700">{alert.client_name}</p><p className="mt-1 text-xs text-zinc-500">{alert.reason}</p></div><div className="flex items-center gap-3 sm:justify-end"><span className="font-mono text-xs text-zinc-500">{formatTime(alert.updated_at || alert.created_at)}</span><ArrowRight size={17} className="text-zinc-400 transition group-hover:translate-x-1" /></div></div>; }
...
function AlertStatusBadge({ status }: { status: AlertStatus }) { const tone = status === "new" ? "bg-rose-50 text-rose-800" : status === "investigating" ? "bg-amber-50 text-amber-800" : status === "client_notified" ? "bg-violet-50 text-violet-800" : status === "monitoring" ? "bg-sky-50 text-sky-800" : status === "resolved" ? "bg-teal-50 text-teal-800" : "bg-zinc-100 text-zinc-600"; return <span className={`w-fit shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${tone}`}>{alertStatusLabel(status)}</span>; }
